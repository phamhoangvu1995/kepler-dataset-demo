...
def _get_plan_to_near_optimal_params(
    results: JSON,
    results_key: str,
    estimator: LatencyEstimator = LatencyEstimator.MIN,
  ...
  estimator_func = ESTIMATOR_MAP[estimator]
...
def execute_training_data_queries(
  ...
  print(f"Processing: {query_id} with {plan_count} plan_hints, skipping {skip_indices}.")
  ...
  for param_index, params in enumerate(reordered_param_sets):
    ...
    print(f"Parameter: {params_as_string}")

    if params_as_string in reexecute_param_strings:
      print("Reexecuting {params_as_string}")
    elif params_as_string in results:
      print("Already executed parameter")
      continue
