...
def verify_hints(
  ...
  """
  print('Checking %s hints for query_id %s' % (len(plan_hints), query_id))
  failure_counts = {}
  ...
  for hint in plan_hints:
    ...
    if params_failure_count:
      print('Failure rate for hint %s source %s: %s/%s' % (
...
class PlanHintExtractor:
  ...
  def add_query_plans(self, params_plans_info: JSON) -> None:
    ...
    if not hints:
      print('Failed to extract hints for %s' % (params_plans_info))
      return
